...
const AboutClient = () => {
    ...
    const fetchStats = async () => {
        try {
            // Fetch job count
            const { count: jobs } = await supabase.from('jobs').select('*', { count: 'exact', head: true }).eq('is_active', true);

            // Fetch company count
            const { count: companies } = await supabase.from('companies').select('*', { count: 'exact', head: true }).eq('is_active', true);
    ...
    const fetchAboutContent = async () => {
        try {
            const { data, error } = await supabase.from('about_content').select('*').eq('is_active', true).order('display_order');
                        ...
                        <CardContent className="text-center">
                            <div className="space-y-4">
                                ...
                                <div className="text-sm text-muted-foreground leading-relaxed max-w-4xl mx-auto space-y-3 pt-4 border-t border-border/30 mt-6">
                                    ...
                                    <p>
                                        Platformamızda IT, maliyyə, satış, marketinq, mühəndislik və digər peşə sahələrində <strong>qiymətlər</strong> tapa bilərsiniz. <Link href="/categories" className="text-primary hover:underline font-medium">Kateqoriyalar</Link> səhifəmizdə müxtəlif sahələr üzrə <strong>iş elanlarını</strong> nəzərdən keçirə, <Link href="/companies" className="text-primary hover:underline font-medium">şirkətlər</Link> bölməsində aparıcı işəgötürənlərlə tanış ola və <Link href="/services" className="text-primary hover:underline font-medium">qiymətləndirmə</Link> səhifəmizdə premium xüsusiyyətlərimiz haqqında məlumat əldə edə bilərsiniz.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                ...
                {featuresData.length > 0 && (
                    <div className="mb-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {/* Referral System */}
                            <Link href="/referral" className="block group">
                                <Card className="h-full border-border/50 shadow-card hover:shadow-card-hover transition-all duration-300 animate-fade-in hover:scale-[1.02] cursor-pointer" style={{
                            ...
                            <Link href="/add_job" className="block group">
                                <Card className="h-full border-border/50 shadow-card hover:shadow-card-hover transition-all duration-300 animate-fade-in hover:scale-[1.02] cursor-pointer" style={{
                            ...
                            <div className="flex flex-wrap justify-center gap-4">
                                {contactData.map((contact: any, index: number) => (
                                    <Badge key={index} variant="outline" className="px-4 py-2 text-sm">
                                        {/* Assuming contact.icon is a string or component, if it's a string we might need to render it differently */}
                                        {contact.value}
                                    </Badge>
                                ))}
                            </div>
