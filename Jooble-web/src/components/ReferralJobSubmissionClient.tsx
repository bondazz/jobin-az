...
const ReferralJobSubmissionClient = () => {
  ...
  const [paymentAmount, setPaymentAmount] = useState<number>(0.01);
  ...
  useEffect(() => {
    ...
    // Fetch job posting price from site_settings
    const fetchPrice = async () => {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        if (data?.value) {
          const price = parseFloat(String(data.value));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Submit the form data with awaiting_payment status
      ...
      const paymentResponse = await fetch(
        "https://igrtzfvphltnoiwedbtz.supabase.co/functions/v1/epoint-create-payment",
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            description: `Jooble.az - İş elanı: ${formData.company_name}`,
            language: "az",
          }),
        }
      );
      ...
      if (paymentResult.success && paymentResult.redirect_url) {
        ...
        toast({
          title: "Ödəniş xətası",
          description: paymentResult.error || "Ödəniş sisteminə qoşulmaq mümkün olmadı",
          variant: "destructive",
        });
      }
    } catch (error) {
                ...
                <CardHeader className="bg-gradient-to-r from-primary/10 via-accent/5 to-primary/10 pb-6">
                  ...
                  <div className="mt-4 p-4 bg-primary/5 rounded-lg border border-primary/10">
                    <p className="text-sm font-medium text-foreground text-center mb-2">
                      İlk vakansiyanız üçün endirimli qiymətlər:
                    </p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">
                          Şirkətiniz haqqında məlumatı formatlaşdıra bilərsiniz.
                        </p>
                        <RichTextEditor
                          value={formData.company_description}
                          onChange={(value) => handleInputChange("company_description", value)}
                          placeholder="Şirkətiniz və ya biznesiniz haqqında məlumat yazın..."
                        />
                      </div>
                    ...
                    <div className="pt-6 border-t border-border">
                      <p className="text-sm text-muted-foreground text-center mb-3">
                        💳 Ödəniş etdikdən sonra vakansiyanız dərc ediləcək
                      </p>
                      <Button
                        type="submit"
                      ...
                      >
                        {isSubmitting ? "Ödəniş səhifəsinə yönləndirilir..." : `Ödəniş et — ${paymentAmount} AZN`}
                      </Button>
                    </div>
                  </form>
                </CardContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">
                          Bu sahə iş elanının əsas məzmunudur. Bold yazılar və digər formatlaşdırma seçimlərini istifadə
                          edə bilərsiniz.
                        </p>
                        <RichTextEditor
                          value={formData.job_article}
