...
const SimilarJobs = ({
  ...
  const formatDate = (dateString: string) => {
    ...
    if (diffDays < 30) return (t('dates.weeksAgo') || `{n} həftə əvvəl`).replace('{n}', Math.floor(diffDays / 7).toString());
    return (t('dates.monthsAgo') || `{n} ay əvvəl`).replace('{n}', Math.floor(diffDays / 30).toString());
  };
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className={`${isMobile ? "text-lg" : "text-xl"} font-bold text-foreground`}>
            {t('jobs.similarJobs') || 'Oxşar Vakansiyalar'}
          </h2>
        </div>
      </div>
      ...
      <div className={`grid ${isMobile ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"} gap-3`}>
        {similarJobs.map((job, index) => {
          ...
          return (
            <Link
              key={job.id}
              href={getLocalizedPath(`/vacancies/${job.slug}`)}
              className={`
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/50">
                {job.salary ? (
                  ...
                  <span className="text-xs text-muted-foreground italic">
                    {t('common.salaryNotSpecified') || 'Maaş göstərilməyib'}
                  </span>
                )}
      ...
      <div className="mt-5 p-4 rounded-xl bg-gradient-to-r from-[#0088cc]/10 to-[#0088cc]/5 dark:from-[#0088cc]/20 dark:to-[#0088cc]/10 border border-[#0088cc]/20">
        <div className="flex items-center justify-center gap-3 flex-wrap text-center">
          ...
          <p className="text-sm text-foreground">
            {t('common.telegramChannel') || 'Vakansiyalar barədə məlumatı ən tez bizim'}{" "}
            <a
