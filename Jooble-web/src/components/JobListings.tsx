...
const JobListings = ({
  ...
  const formatDate = useCallback((dateString: string) => {
    ...
    if (diffDays === 0) return t('dates.today') || 'Bu gün';
    if (diffDays === 1) return t('dates.yesterday') || 'Dünən';
    if (diffDays <= 7) return (t('dates.daysAgo') || '{n} gün əvvəl').replace('{n}', diffDays.toString());
    if (diffDays <= 30) return (t('dates.weeksAgo') || '{n} həftə əvvəl').replace('{n}', Math.ceil(diffDays / 7).toString());
    return (t('dates.monthsAgo') || '{n} ay əvvəl').replace('{n}', Math.ceil(diffDays / 30).toString());
  }, [t]);
  ...
  }, [companyId, offset, jobs.length, formatDate]);
    ...
    <div className="flex-1 flex flex-col h-full bg-background">
          ...
          <div className="relative z-10 p-2 sm:p-3">
            <div className="flex flex-row gap-2 items-center justify-center max-w-4xl mx-auto">
              <div className="relative flex-1 min-w-0">
                ...
                <Input placeholder={t('common.searchJobs') || "İş axtarın..."} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-7 sm:pl-10 bg-background/60 backdrop-blur-sm border-border/60 focus:border-primary/60 text-xs sm:text-sm h-8 sm:h-9 w-full" />
              </div>
              <div className="relative flex-1 min-w-0">
                <MapPin className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground w-3 h-3 sm:w-4 sm:h-4" />
                <Input placeholder={t('common.city') || "Şəhər"} value={locationFilter} onChange={e => setLocationFilter(e.target.value)} className="pl-7 sm:pl-10 bg-background/60 backdrop-blur-sm border-border/60 focus:border-primary/60 text-xs sm:text-sm h-8 sm:h-9 w-full" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 bg-gradient-to-b from-transparent to-primary/5 w-full mx-auto lg:min-h-[calc(100vh-200px)] lg:px-2 xl:px-3 py-0">
        <div className="flex flex-col gap-2 justify-center items-center w-full px-1.5 sm:px-2 py-[12px] lg:max-w-full mx-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground animate-fade-in">
              ...
              <h3 className="text-lg font-bold mb-2 text-foreground">{t('common.noJobsFound') || 'Heç bir iş tapılmadı'}</h3>
              <p className="text-sm text-center mb-4 max-w-sm leading-relaxed">
                {t('common.noJobsDescription') || 'Axtarış kriteriyalarınızı dəyişdirin və ya fərqli kateqoriyaları araşdırın'}
              </p>
              <Button variant="outline" className="mt-2 px-4 py-2.5 rounded-lg border-primary/30 text-primary hover:bg-primary hover:text-white transition-all duration-300 hover:shadow-lg font-medium text-sm" onClick={() => {
                ...
                {t('common.clearFilters') || 'Süzgəcləri təmizlə'}
              </Button>
            </div>
          )}
        </div>
        ...
        {!loading && filteredJobs.length > 0 && (
          <nav aria-label={t('common.vacancies') || 'Vakansiyalar'} className="flex justify-center items-center gap-2 py-6 border-t border-border/30 mt-4">
            {/* Previous page link */}
            {displayCount > 15 && (
              <Link 
                href={getLocalizedPath(`/vacancies?page=${Math.max(1, Math.floor(displayCount / 15) - 1)}`)}
              ...
              >
                ...
                <span className="hidden sm:inline">{t('common.previous') || 'Əvvəlki'}</span>
              </Link>
            )}
            ...
            {(() => {
              ...
              for (let i = startPage; i <= endPage; i++) {
                pages.push(
                  <Link
                    key={i}
                    href={getLocalizedPath(`/vacancies?page=${i}`)}
            ...
            {displayCount < filteredJobs.length && (
              <Link
                href={getLocalizedPath(`/vacancies?page=${Math.floor(displayCount / 15) + 1}`)}
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-primary transition-colors rounded-md border border-border/50 hover:border-primary/30"
              >
                <span className="hidden sm:inline">{t('common.next') || 'Sonrakı'}</span>
