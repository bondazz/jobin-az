...
export default function AdminSettingsClient() {
  ...
  const getSettingDescription = (key: string): string => {
    const descriptions: { [key: string]: string } = {
      ...
      job_posting_price: 'Vakansiya yerləşdirmə qiyməti (AZN)',
    };
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="site" className="w-full">
          <TabsList className="grid w-full grid-cols-7">
            ...
            <TabsTrigger value="job_price" className="flex items-center gap-2">
              <DollarSign className="w-4 h-4" />
              Elan Qiyməti
            </TabsTrigger>
          </TabsList>
          ...
          {/* Job Posting Price */}
          <TabsContent value="job_price" className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <DollarSign className="h-5 w-5" />
                  Vakansiya Yerləşdirmə Qiyməti
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="job_posting_price">Qiymət (AZN)</Label>
                  <Input
                    id="job_posting_price"
                    type="number"
                    step="0.01"
                    min="0"
                    value={settings.job_posting_price || '0.01'}
                    onChange={(e) => handleInputChange('job_posting_price', e.target.value)}
                    placeholder="0.01"
                  />
                  <p className="text-sm text-muted-foreground">
                    Bu qiymət /add_job səhifəsində istifadəçilərə göstəriləcək və ödəniş zamanı tətbiq olunacaq.
                  </p>
                </div>

                <div className="p-4 bg-muted/50 rounded-lg border border-border">
                  <p className="text-sm text-muted-foreground">
                    <strong>Cari qiymət:</strong>{' '}
                    <span className="text-primary font-semibold text-lg">
                      {settings.job_posting_price || '0.01'} AZN
                    </span>
                  </p>
                </div>
