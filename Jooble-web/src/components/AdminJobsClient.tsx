...
import { Plus, Edit, Trash2, Search, Eye, Building2, DollarSign, RefreshCw, Calendar, Globe } from "lucide-react";
...
export default function AdminJobsClient() {
  ...
  const fetchData = async () => {
    try {
      ...
      const categoriesResponse = await supabase.from("categories").select("id, name").eq("is_active", true);
  ...
  useEffect(() => {
    ...
    const keyword = slugKeywords[Math.floor(Math.random() * slugKeywords.length)];

    // Generate unique 7-digit number
    const uniqueNumber = Math.floor(1000000 + Math.random() * 9000000);
    const uniqueSlug = `${keyword}-${companySlug}-${titleSlug}-${uniqueNumber}`;

    setFormData((prev) => {
      const prevSeoTitle = prev.seo_title ?? "";
      const prevSeoDesc = prev.seo_description ?? "";
      const shouldUpdateTitle =
        !seoTitleEdited && (prevSeoTitle.trim() === "" || prevSeoTitle === lastGeneratedSEO.title);
      const shouldUpdateDesc =
        !seoDescEdited && (prevSeoDesc.trim() === "" || prevSeoDesc === lastGeneratedSEO.description);
    ...
    try {
      ...
      if (editingJob) {
        const { data: updatedJob, error } = await supabase
          ...
          .update(jobData)
          .eq("id", editingJob.id)
          .select(`*, companies (name), categories (name)`)
          .single();

        if (error) throw error;

        // Update in-place without re-fetching — preserves list order & scroll
        setJobs((prev) =>
          prev.map((j) => (j.id === editingJob.id ? (updatedJob as Job) : j))
        );
    ...
    <>
      <div className="p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingJob ? "Vakansiya Redaktə Et" : "Yeni Vakansiya Əlavə Et"}</DialogTitle>
              </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value: "website" | "email") =>
        ...
        <div className="border rounded-lg">
          <List height={800} itemCount={filteredJobs.length} itemSize={60} width="100%">
            {({ index, style }) => {
              ...
              return (
                <div style={style} className="px-2">
                  <Card
                    className={`hover:shadow-md transition-shadow h-[56px] ${expired ? "border-orange-200 bg-orange-50/50" : ""}`}
                  >
                    <CardContent className="p-3">
                      <div className="flex justify-between items-center gap-4">
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          <Button size="sm" variant="outline" onClick={() => handleEdit(job)} className="h-7 w-7 p-0">
