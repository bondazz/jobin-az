...
const HomeClient = () => {
    ...
    const formatDate = (dateString: string) => {
        ...
        if (diffDays <= 7) return t('dates.daysAgo').replace('{n}', String(diffDays - 1));
        if (diffDays <= 30) return t('dates.weeksAgo').replace('{n}', String(Math.ceil(diffDays / 7)));
        return t('dates.monthsAgo').replace('{n}', String(Math.ceil(diffDays / 30)));
    };

    const handleJobSelect = useCallback((job: Job) => {
        // Update URL and state without scrolling or full page reload
        if (job.slug) {
            const urlWithReferral = getUrlWithReferral(getLocalizedPath(`/vacancies/${job.slug}`));
            router.push(urlWithReferral, { scroll: false });
        }
        setSelectedJob(job);
    }, [router, getUrlWithReferral, getLocalizedPath]);
                                ...
                                {selectedCategory && (
                                    <div className="flex items-center gap-1 bg-primary/20 text-primary text-xs px-2 py-1 rounded">
                                        ...
                                        <button
                                            onClick={() => {
                                                setSelectedCategory('');
                                            router.push(getLocalizedPath('/vacancies'));
                                        }}
