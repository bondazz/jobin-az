...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const description = job.seo_description || `Вакансия ${job.title} в компании ${companyName}. Подайте заявку сейчас!`;
    ...
    return {
        ...
        openGraph: {
            type: 'website',
            url: `https://jooble.az/ru/vacancies/${params.jobSlug}`,
            title,
            description,
            siteName: 'Jooble.az',
        ...
        alternates: {
            canonical: `https://jooble.az/ru/vacancies/${params.jobSlug}`,
            languages: {
...
export default async function RuJobPage({ params }: Props) {
    const { data: job } = await supabase
        .from('jobs')
        .select(`
    ...
    const breadcrumbSchema = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 2, "name": "Вакансии", "item": "https://jooble.az/ru/vacancies" },
            { "@type": "ListItem", "position": 3, "name": job.title, "item": `https://jooble.az/ru/vacancies/${job.slug}` }
        ]
    };
    ...
    return (
        <>
            ...
            <div className="sr-only">
                ...
                {job.salary && <p>Зарплата: {job.salary}</p>}
