...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const title = category.seo_title || `${category.name} Jobs | ${jobsCount} Vacancies - Jooble.az`;
    const description = category.seo_description || `${jobsCount} active job vacancies in ${category.name}. Find the latest job opportunities.`;

    return {
        title,
        description,
        alternates: {
            canonical: `https://jooble.az/en/categories/${category.slug}`,
            languages: {
        ...
        openGraph: {
            title,
            description,
            url: `https://jooble.az/en/categories/${category.slug}`,
            siteName: 'Jooble.az',
            type: 'website',
        },
    };
...
export default async function EnCategoryPage({ params }: Props) {
    ...
    const breadcrumbJsonLd = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Categories', item: 'https://jooble.az/en/categories' },
            { '@type': 'ListItem', position: 3, name: category.name, item: `https://jooble.az/en/categories/${category.slug}` }
        ]
    };
    ...
    return (
        <>
            ...
            <div className="sr-only">
                ...
                <p>{jobsCount} active vacancies in {category.name}</p>
            </div>
            <CategoriesClient />
        </>
    );
