...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const { company } = data;
    const description = company.seo_description || stripHtml(company.description || '') || `Information about ${company.name} and job vacancies.`;

    return {
        title: company.seo_title || `${company.name} | Jooble`,
        description: description.slice(0, 160),
        keywords: company.seo_keywords?.join(', ') || `${company.name}, vacancies, jobs`,
        openGraph: {
            title: company.seo_title || `${company.name} | Jooble`,
            description: description.slice(0, 160),
            images: company.logo ? [company.logo] : [],
            url: `https://jooble.az/en/companies/${params.company}`,
            type: 'website',
        },
        alternates: {
            canonical: `https://jooble.az/en/companies/${params.company}`,
            languages: {
...
export default async function EnCompanyPage({ params }: Props) {
    const data = await getCompanyData(params.company);
    ...
    const organizationJsonLd = {
        ...
        '@type': 'Organization',
        name: company.name,
        description: plainDescription || `${company.name} company`,
        url: `https://jooble.az/en/companies/${company.slug}`,
    ...
    const breadcrumbJsonLd = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Companies', item: 'https://jooble.az/en/companies' },
            { '@type': 'ListItem', position: 3, name: company.name, item: `https://jooble.az/en/companies/${company.slug}` }
        ]
    };
    ...
    return (
        <>
            ...
            <div className="sr-only">
                <h1>{company.name}</h1>
                <p>Active vacancies: {jobCount}</p>
                {jobs.length > 0 && (
                    <ul>
                        {jobs.map((job) => (
                            <li key={job.slug}>
                                <a href={`https://jooble.az/en/vacancies/${job.slug}`}>{job.title}</a>
                            </li>
                        ))}
                    </ul>
                )}
