...
export async function generateMetadata({ params }: RegionPageProps): Promise<Metadata> {
    ...
    if (!data) {
        return { title: 'Region not found - Jooble.az' };
    }

    const { region, jobCount } = data;
    const title = region.seo_title || `${region.name} Jobs (${jobCount} Vacancies) | Jooble.az`;
    const description = region.seo_description || `${jobCount}+ job vacancies in ${region.name}. Find the best opportunities.`;

    return {
        title,
        description,
        alternates: {
            canonical: `https://jooble.az/en/regions/${region.slug}`,
            languages: {
...
export default async function EnRegionPage({ params }: RegionPageProps) {
    const data = await getRegionData(params.region);
    ...
    const breadcrumbJsonLd = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Regions', item: 'https://jooble.az/en/regions' },
            { '@type': 'ListItem', position: 3, name: region.name, item: `https://jooble.az/en/regions/${region.slug}` }
        ]
    };
    ...
    return (
        <>
            ...
            <div className="sr-only">
                <h1>{region.name} Job Vacancies</h1>
                <p>{jobCount} active jobs in {region.name}</p>
            </div>
            <RegionsClient />
        </>
    );
